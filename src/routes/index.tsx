...
const CAMILA_PHOTO = "/camila-nunes.png";
...
const faqs = [
  ...
  ["O atendimento é online?", "Sim. O atendimento psicológico é realizado online, com praticidade e privacidade para que você tenha seu espaço de cuidado de onde estiver."],
...
const PsychologyMark = () => <img src="/psicologia.svg" alt="Símbolo da Psicologia" className="psychology-mark" />;
    ...
    <div className="site-shell">
      <header className="site-header">
        <div className="container header-inner">
          <a className="brand" href="#inicio" onClick={closeMenu}><PsychologyMark /><span className="brand-name">Camila Nunes</span><span className="brand-role">Psicóloga</span></a>
          <button className="menu-button" type="button" onClick={() => setMenuOpen(!menuOpen)} aria-label={menuOpen ? "Fechar menu" : "Abrir menu"} aria-expanded={menuOpen}><span /><span /></button>
          <nav className={menuOpen ? "main-nav is-open" : "main-nav"} aria-label="Navegação principal">
            <a href="#inicio" onClick={closeMenu}>Início</a><a href="#sobre" onClick={closeMenu}>Sobre</a><a href="#atendimento" onClick={closeMenu}>Atendimento</a><a href="#processo" onClick={closeMenu}>Como funciona</a><a href="#duvidas" onClick={closeMenu}>Dúvidas</a><a className="header-cta" href={WHATSAPP} target="_blank" rel="noreferrer" onClick={closeMenu}>Agendar conversa</a>
          </nav>
        </div>
      </header>
      ...
      <main>
        <section className="hero" id="inicio">
          <div className="container hero-grid">
            <div className="hero-content"><p className="overline">Psicologia clínica</p><h1>Um lugar seguro para <i>entender o que você vive.</i></h1><p className="hero-lead">A terapia pode ser o espaço que faltava para desacelerar, colocar em palavras o que você sente e olhar para sua história com mais clareza.</p><div className="hero-buttons"><a className="primary-button" href={WHATSAPP} target="_blank" rel="noreferrer">Conversar com a Camila</a><a className="secondary-link" href="#sobre">Conheça meu trabalho <span>→</span></a></div><div className="hero-note"><span>Atendimento psicológico individual</span><span>Online</span></div></div>
            <div className="hero-photo-area"><div className="photo-card"><img src={CAMILA_PHOTO} alt="Camila Nunes, psicóloga" className="camila-photo" loading="eager" decoding="async" /><div className="photo-label"><strong>Camila Nunes</strong><span>Psicóloga</span></div></div><div className="photo-side-text">ESCUTA<br />PRESENÇA<br />CUIDADO</div></div>
          </div><div className="hero-bottom-line" />
        </section>
